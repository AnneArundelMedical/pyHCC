...
def load_facts():
  load_cc_facts("src\\content\\icd10.txt",0)
  load_cc_facts("src\\content\\icd9.txt",9)
  load_hcc_facts()
  load_diagnostic_category_facts()
  load_coefficients("src\\content\\coefficients.txt")
...
def load_rules():
  ...
  edit(ICD,9,B,"112") <= age(B,A)  & (A < 18) & (ICD.in_(["4910", "4911", "49120", "49121", "49122", "4918", "4919", "4920",  "4928",  "496",  "5181", "5182"]))
  edit(ICD,0,B,"48")  <= female(B) & (ICD.in_(["D66", "D67"]))
  edit(ICD,0,B,"112") <= age(B,A)  & (A < 18) & (ICD.in_(["J410", "J411", "J418", "J42",  "J430", "J431", "J432", "J438", "J439", "J440",
                                 "J441", "J449", "J982", "J983"]))
  ...
  beneficiary_has_cc(B,CC) <= beneficiary_icd(B,ICD,Type)  & cc(ICD,CC,Type) & ~(edit(ICD,Type,B,CC2)) & ~(excised(ICD,Type,B))
  ...
  (new_enrollee_score[B] == sum_(Coef,key=Coef)) <=  indicator(B,CC) & CC.in_(nevars) & coefficient("NE_"+CC,Coef)
  (institutional_score[B] == sum_(Coef,key=Coef)) <=  indicator(B,CC) & CC.in_(ivars) & coefficient("INS_"+CC,Coef)
  (community_score[B] == sum_(Coef,key=Coef)) <=  indicator(B,CC) & CC.in_(cvars) & coefficient("CE_"+CC,Coef)
...
def get_hcc_score(bene, model_name):
  return float(score(bene,model_name,X)[0][0])

def get_all_hcc_scores(bene):
  score_community = get_hcc_score(bene,"community")
  score_institutional = get_hcc_score(bene,"institutional")
  score_new_enrollee = get_hcc_score(bene,"new_enrollee")
  return {"community": score_community
          ,"institutional": score_institutional
          ,"new_enrollee": score_new_enrollee}
